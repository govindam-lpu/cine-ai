...
class LetterboxdScraper:
    ...
    def _fetch_with_backoff(self, url: str) -> requests.Response:
        for attempt in range(4):
            ...
            hard_blocked = response.status_code == 403 or "just a moment" in text or "enable javascript and cookies" in text
    ...
    def _entry_from_poster_node(div) -> FilmEntry | None:
        ...
        return FilmEntry(title=title, year=year, letterboxd_slug=f"/film/{slug}/", user_rating=rating, is_rewatch=rewatch)
    ...
    def _scrape_rss_fallback(self, username: str) -> list[FilmEntry]:
        ...
        for item in root.findall("./channel/item"):
            ...
            watched_at = self._parse_rss_datetime(watched_raw) or self._parse_rss_datetime(self._xml_text(item, "pubDate"))
