...
import type { AntiRecommendation, Recommendation, RecommendationPayload } from "@/types/recommendation";
...
export function useRecommendations(user: UserProfile | null, mood = "") {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      wildCard: payload?.wild_card ?? undefined,
      antiRecommendation: payload?.anti_recommendations?.[0] as AntiRecommendation | undefined,
      headerNote: payload
